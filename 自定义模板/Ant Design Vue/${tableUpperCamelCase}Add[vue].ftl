<#-- 用于生成 Ant Design for Vue 新增页面的自定义模板 -->
<#-- 初始化Form表单字段 -->
<#assign formFeilds = FtlUtils.getJsonFieldList(jsonParam.formFeilds, tableInfo.tableName) />

<#if tableInfo.simpleRemark?has_content><!-- ${tableInfo.simpleRemark}新增页面 --></#if>
<template>
  <a-card>
    <a-form :form="form">
      <a-row :gutter="48">
...
import { save${tableInfo.upperCamelCase} } from '@/api/${jsonParam.basePath}/${tableInfo.lowerCamelCase}'
    ...
    handleSubmit (type) {
      ...
      validateFields((err, values) => {
        if (!err) {
          ...
          save${tableInfo.upperCamelCase}(values).then(res => {
            // TODO: 处理结果数据
            ...
            console.error('save${tableInfo.upperCamelCase}', e)
          })
        }
      })
    }
