...
4、JSON参数配置文件中的searchFields用于在Condition类中添加查询条件；
   batchSearchFields用于字段列表的批量查询；
   checkValueExistedFields用于生成检查字段值是否已存在的接口，即生成检查逻辑唯一的接口，在新增和修改数据时使用；
   importAndExportFields用于生成表数据的导入导出接口；
