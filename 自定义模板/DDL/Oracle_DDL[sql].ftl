...
(
        <#list tableInfo.fieldInfos as fieldInfo>
${"    ${fieldInfo.colName}"?right_pad(30)} <#if StringUtils.equalsIgnoreCase(fieldInfo.typeName, "varchar")>VARCHAR2(${fieldInfo.length})<#elseif StringUtils.equalsIgnoreCase(fieldInfo.typeName, "decimal")>NUMBER(${fieldInfo.numberPrecision})<#elseif StringUtils.equalsIgnoreCase(fieldInfo.typeName, "numeric")>NUMBER(${fieldInfo.numberPrecision})<#elseif StringUtils.equalsIgnoreCase(fieldInfo.typeName, "double")>NUMBER(${fieldInfo.numberPrecision})<#elseif StringUtils.equalsIgnoreCase(fieldInfo.typeName, "int")>NUMBER(10,0)<#elseif StringUtils.equalsIgnoreCase(fieldInfo.typeName, "int unsigned")>NUMBER(20,0)<#elseif StringUtils.containsIgnoreCase(fieldInfo.typeName, "tinyint")>NUMBER(3,0)<#elseif StringUtils.containsIgnoreCase(fieldInfo.typeName, "smallint")>NUMBER(5,0)<#elseif StringUtils.containsIgnoreCase(fieldInfo.typeName, "mediumint")>NUMBER(7,0)<#elseif StringUtils.containsIgnoreCase(fieldInfo.typeName, "bigint")>NUMBER(20,0)<#elseif StringUtils.equalsIgnoreCase(fieldInfo.typeName, "int2")>NUMBER(5,0)<#elseif StringUtils.equalsIgnoreCase(fieldInfo.typeName, "int4")>NUMBER(10,0)<#elseif StringUtils.equalsIgnoreCase(fieldInfo.typeName, "int8")>NUMBER(20,0)<#elseif StringUtils.equalsIgnoreCase(fieldInfo.typeName, "datetime")>DATE<#else>${fieldInfo.dataTypeStr}</#if><#if fieldInfo.defaultValue?has_content> DEFAULT <#if fieldInfo.isNumericType>${fieldInfo.defaultValue}<#else>'${fieldInfo.defaultValue}'</#if></#if><#if fieldInfo.isNotNull> NOT NULL</#if><#if pkCount &gt; 0 || ukCount &gt; 0>,<#elseif fieldInfo_has_next>,</#if>
        </#list>
        <#if pkCount &gt; 0>
    CONSTRAINT PK_${tableInfo.upperTableName} PRIMARY KEY (<#list tableInfo.fieldInfos as fieldInfo><#if fieldInfo.primaryKey>${fieldInfo.colName}<#assign pkCount = pkCount - 1 /></#if><#if pkCount &gt; 0>, </#if></#list>)<#if ukCount &gt; 0>,</#if>
        </#if>
        <#if ukCount &gt; 0>
            <#list tableInfo.fieldInfos as fieldInfo>
