...
(
        <#list tableInfo.fieldInfos as fieldInfo>
${"    ${fieldInfo.colName}"?right_pad(30)} <#if StringUtils.equalsIgnoreCase(fieldInfo.typeName, "varchar2")>varchar(${fieldInfo.length})<#elseif StringUtils.equalsIgnoreCase(fieldInfo.typeName, "nvarchar")>varchar(${fieldInfo.length})<#elseif StringUtils.containsIgnoreCase(fieldInfo.typeName, "nvarchar2")>VARCHAR(${fieldInfo.length})<#elseif StringUtils.containsIgnoreCase(fieldInfo.typeName, "nchar")>CHAR(${fieldInfo.length})<#elseif StringUtils.equalsIgnoreCase(fieldInfo.typeName, "number")>decimal(${fieldInfo.numberPrecision})<#elseif StringUtils.equalsIgnoreCase(fieldInfo.typeName, "double")>decimal(${fieldInfo.numberPrecision})<#elseif StringUtils.containsIgnoreCase(fieldInfo.typeName, "tinyint")>int2<#elseif StringUtils.containsIgnoreCase(fieldInfo.typeName, "smallint")>int2<#elseif StringUtils.containsIgnoreCase(fieldInfo.typeName, "mediumint")>int4<#elseif StringUtils.equalsIgnoreCase(fieldInfo.typeName, "int")>int4<#elseif StringUtils.equalsIgnoreCase(fieldInfo.typeName, "int unsigned")>int8<#elseif StringUtils.equalsIgnoreCase(fieldInfo.typeName, "integer")>int4<#elseif StringUtils.containsIgnoreCase(fieldInfo.typeName, "bigint")>int8<#elseif StringUtils.equalsIgnoreCase(fieldInfo.typeName, "long")>int8<#elseif StringUtils.equalsIgnoreCase(fieldInfo.typeName, "datetime")>date<#else>${fieldInfo.dataTypeStr}</#if><#if fieldInfo.isNotNull> NOT NULL</#if><#if fieldInfo.defaultValue?has_content> DEFAULT <#if fieldInfo.isNumericType>${fieldInfo.defaultValue}<#else>'${fieldInfo.defaultValue}'</#if></#if><#if pkCount &gt; 0 || ukCount &gt; 0>,<#elseif fieldInfo_has_next>,</#if>
        </#list>
        <#if pkCount &gt; 0>
    CONSTRAINT pk_${tableInfo.lowerTableName} PRIMARY KEY (<#list tableInfo.fieldInfos as fieldInfo><#if fieldInfo.primaryKey>${fieldInfo.colName}<#assign pkCount = pkCount - 1 /></#if><#if pkCount &gt; 0>, </#if></#list>)<#if ukCount &gt; 0>,</#if>
        </#if>
        <#if ukCount &gt; 0>
            <#list tableInfo.fieldInfos as fieldInfo>
