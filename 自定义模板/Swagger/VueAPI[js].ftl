<#-- 生成Vue后台JS接口的自定义模板 -->
<#if jsonParam?has_content && jsonParam.basePath?has_content>
    <#assign basePath = jsonParam.basePath />
</#if>
...
export function ${path.name} (<#if paramsExisted>parameter</#if>) {
    return axios({
        ...
        url: '${basePath}${path.path?substring(0, StringUtils.lastIndexOf(path.path,"/"))}/' + parameter
        <#else>
        url: '${basePath}${path.path}'<#if paramsExisted && paramsType != "path">,</#if>
        </#if>
