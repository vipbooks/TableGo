...
<#if checkValueExistedFields?has_content>
     ...
     * @return 是否存在
     */
    String check${fieldInfo.upperCamelCase}Existed(@Param("${fieldInfo.proName}") ${fieldInfo.javaType} ${fieldInfo.proName}<#if tableInfo.pkLowerCamelName??>, @Param("${tableInfo.pkLowerCamelName}") ${tableInfo.pkJavaType} ${tableInfo.pkLowerCamelName}</#if>);
            </#if>
        </#list>
    </#list>
</#if>
